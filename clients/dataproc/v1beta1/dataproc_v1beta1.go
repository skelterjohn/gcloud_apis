...
// Usage example:
//
//   import "github.com/skelterjohn/gcloud_apis/clients/dataproc/v1beta1"
//   ...
...
type Agent struct {
	...
	// [a-z0-9.-]{1,64}
	// An agent chosen ID. This should typically be the hostname of the
	// GCE
	// virtual machine on which the agent is currently running.
	...
	//   "INITIALIZING" - Agent has started and is running built-in
...
type ClusterConfiguration struct {
	...
	// SSH keys and configuration. If you do not specify a staging bucket,
	// Cloud
	// Dataproc will determine an appropriate Cloud Storage location
	// (US,
	...
	// nodes.
	// You can test a node's <code>role</code> metadata to run an executable
	// on
...
// DiagnosticRecord: A record that can be used for diagnosing agent
// state.
type DiagnosticRecord struct {
	...
	// ForceSendFields is a list of field names (e.g. "ErrorSample") to
	// unconditionally include in API requests. By default, fields with
	// empty values are omitted from API requests. However, any non-pointer,
	// non-interface field appearing in ForceSendFields will be sent to the
	// server regardless of whether the field is empty or not. This may be
...
func (s *DiagnosticRecord) MarshalJSON() ([]byte, error) {
	type noMethod DiagnosticRecord
	raw := noMethod(*s)
	return gensupport.MarshalJSON(raw, s.ForceSendFields)
}
...
type DiskConfiguration struct {
	...
	// (default is 0).
	// If SSDs are not attached, the boot disk is used to store runtime logs
	// and
...
type GceClusterConfiguration struct {
	...
	// If no scopes are specfied, the following defaults are also
	// provided:
...
type InstanceGroupConfiguration struct {
	...
	// Can actually be specified for development purposes
	// Example:
	...
	// eventually support high availability with multiple masters.
...
type Job struct {
	...
	// which may be used as part of job setup and handling. If not
	// present,
	...
	// can be used to
	// obtain the equivalent REST path of the job resource. If this
	// property
	// is not specified when a job is created, the server generates
	// a
	// <code>job_id</code>.
...
// This will also eventually
// store the information for scheduling and queueing of jobs.
...
type JobReference struct {
	...
	// project. The job ID
	// is generated by the server upon job submission or provided by the
	// user as a
	// means to perform retries without creating duplicate jobs. The ID
	// must
...
// May eventually contain information
// about progress, session state, etc.
...
type ListJobsResponse struct {
	...
	// to fetch. To fetch additional results, provide this value as
	// the
...
type NodeInitializationAction struct {
	...
	// complete. Default is
	// 10 minutes. Cluster creation fails with an explanatory error message
	// (the
...
type QueryList struct {
	...
	// by separating each with a semicolon. Here is an example of an
	// Cloud
...
type SystemTaskStatus struct {
	...
	//   "PENDING" - Reconfiguration has not started, but request has
	// been
...
// Task: A single Task for agent execution.
//
// A task in Cloud Dataproc is a unit of work originating with the Cloud
// Dataproc service
// and intended for execution on a Cloud Dataproc agent. The most
// straightforward
// example of a Task in Cloud Dataproc is one corresponding to an
// end-user originated
// Job. This task is then allocated to an available agent for execution.
// Other
// examples of Cloud Dataproc Tasks could include executing
// system-maintenance
// scripts, periodic agent upgrades, etc.
//
// Using the example of a Task corresponding to a Job, the flow
// through
// Cloud Dataproc will appear as:
//
// - The end-user submits a Job to Cloud Dataproc for execution.
// - The Cloud Dataproc service determines the best Cluster to execute
// the Job on
//    and creates a Task for the job.
// - An agent polls Cloud Dataproc for outstanding Tasks via the
// LeaseTasks
//    method and the Task created in step 2 is provided to the agent.
// - During execution of the Task, the agent updates the Task status
// via
//    the ReportTaskStatus method.
// - Upon completion of a Task, the agent will finally ReportTaskStatus
//    with a TaskStatus indicating the status of the driver
// application's
//    exit status.
// - The Cloud Dataproc service updates the status of the user-submitted
// Job
//    using the Task status reported by the agent.
type Task struct {
...
// for Master node
// to update configuration based on provided values.
type TaskClusterConfiguration struct {
	...
	//
	// Possible values:
	//   "TYPE_UNSPECIFIED" - No type specified. This is an invalid
	// configuration.
	//   "TYPE_CLUSTER_MEMBERSHIP_CHANGE" - Adding or removing nodes to
	// cluster.
...
type TaskJobConfiguration struct {
	...
	// as a posix-style
	// username. If the user is not a member of the system's
	// user-database,
...
type TaskJobStatus struct {
	...
	//   "PENDING" - The driver has not yet been started, but it's known to
	// the agent.
	//   "RUNNING" - The driver is running.
	//   "KILL_PENDING" - A kill command has been received and the agent is
	// shutting down
	// the driver.
...
// run a
// maintenance command on targeted agent.
...
type TaskQueryList struct {
	...
	// task-type dependent,
	// but in each case each query should be executed within its
	// own
...
// of
// <code>org.apache.hadoop.yarn.proto.YarnProtos.ApplicationReportProt
// o</code>.
type YarnApplication struct {
	...
	// URL uses
	// the internal hostname, and requires a proxy server for resolution
	// and,
	// possibly, access.
	TrackingUrl string `json:"trackingUrl,omitempty"`
...
// in a cluster to 5, the <code>update_mask</code> parameter would
// be
// specified as
// <code>configuration.worker_configuration.num_instances</code>,
// and the `PATCH` request body would specify the new value, as
// follows:
//
//     {
//       "configuration":{
...
// and all agent-owned resources and tasks are free to be
// re-distributed.
...
// any state
// and each agent is expected to start any non-started tasks and to
// monitor
// any YarnApplications spawned by any already running tasks. It's
// expected
// that monitoring previously launched tasks will be more prevalent
// when
// drivers are run entirely within YARN containers.
//
// While there's a single lease expiration time, in the event of
// multiple
// tasks being leased to the agent in a single call, each task has
// a
// unique lease and status must be reported before the lease times out
// or
// the task can be considered orphaned.
//
// The service will determine how many tasks to lease to agents in a
// single
// call.
