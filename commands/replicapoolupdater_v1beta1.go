...
func Replicapoolupdater_v1beta1_RolloutAbandon(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		pathParams = append(pathParams, commands_util.AngrySnakes("project"))
		pathParams = append(pathParams, commands_util.AngrySnakes("zone"))
		pathParams = append(pathParams, commands_util.AngrySnakes("rollout"))

		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts/{rollout}/abandon", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
	...
	service := api_client.NewRolloutService(api_service)
	...
	param_rollout, err := commands_util.ConvertValue_string(paramValues[2])
	if err != nil {
		return err
	}

	call := service.Abandon(param_project, param_zone, param_rollout)
...
func Replicapoolupdater_v1beta1_RolloutCommit(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts/{rollout}/commit", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
	...
	call := service.Commit(param_project, param_zone, param_rollout)
...
func Replicapoolupdater_v1beta1_RolloutGet(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts/{rollout}", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
	...
	call := service.Get(param_project, param_zone, param_rollout)
...
func Replicapoolupdater_v1beta1_RolloutInsert(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
		...
		usageBits += " [--updatePolicyInitialisationMethod=VALUE]"
	...
	queryParamNames := map[string]bool{
		"updatePolicyInitialisationMethod": false,
	}
	...
	// Set query parameters.
	if value, ok := flagValues["updatePolicyInitialisationMethod"]; ok {
		query_updatePolicyInitialisationMethod, err := commands_util.ConvertValue_string(value)
		if err != nil {
			return err
		}
		call.UpdatePolicyInitialisationMethod(query_updatePolicyInitialisationMethod)
	}
...
func Replicapoolupdater_v1beta1_RolloutList(context Context, args ...string) error {
...
func Replicapoolupdater_v1beta1_RolloutPause(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts/{rollout}/pause", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
	...
	call := service.Pause(param_project, param_zone, param_rollout)
...
func Replicapoolupdater_v1beta1_RolloutRampup(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts/{rollout}/rampup", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
		...
		fmt.Fprintf(os.Stderr, "Usage:\n\t%s\n", usageBits)
		commands_util.PrintRequestExample(&api_client.RampUpRolloutRequest{})
	...
	request := &api_client.RampUpRolloutRequest{}
	if len(args) == 2 {
		err = commands_util.PopulateRequestFromFilename(&request, args[1])
		if err != nil {
			return err
	...
	call := service.Rampup(param_project, param_zone, param_rollout,
		request,
	)
...
func Replicapoolupdater_v1beta1_RolloutResume(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts/{rollout}/resume", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
	...
	call := service.Resume(param_project, param_zone, param_rollout)
...
func Replicapoolupdater_v1beta1_RolloutRollback(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{project}/zones/{zone}/rollouts/{rollout}/rollback", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
	...
	call := service.Rollback(param_project, param_zone, param_rollout)
