...
import (
	...
	api_client "github.com/skelterjohn/gcloud_apis/clients/bio/v1"
...
func Bio_v1_ProjectsOperationsCancel(context Context, args ...string) error {
...
func Bio_v1_ProjectsOperationsGet(context Context, args ...string) error {
...
func Bio_v1_ProjectsOperationsList(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("v1/{+name}/operations", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
...
func Bio_v1_ProjectsPipelinesRunDeepVariantV1alpha(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("v1/projects/{projectId}/pipelines:runDeepVariantV1alpha", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
		...
		fmt.Fprintf(os.Stderr, "Usage:\n\t%s\n", usageBits)
		commands_util.PrintRequestExample(&api_client.RunDeepVariantV1alphaRequest{})
	...
	service := api_client.NewProjectsPipelinesService(api_service)
	...
	request := &api_client.RunDeepVariantV1alphaRequest{}
	if len(args) == 2 {
	...
	call := service.RunDeepVariantV1alpha(param_projectId,
		request,
	)
