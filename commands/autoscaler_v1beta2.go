...
import (
	...
	api_client "github.com/skelterjohn/gcloud_apis/clients/autoscaler/v1beta2"
...
func Autoscaler_v1beta2_AutoscalersDelete(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("projects/{project}/zones/{zone}/autoscalers/{autoscaler}", "+") {
...
func Autoscaler_v1beta2_AutoscalersInsert(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("projects/{project}/zones/{zone}/autoscalers", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
