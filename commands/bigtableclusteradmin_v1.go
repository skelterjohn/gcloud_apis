...
import (
	...
	api_client "github.com/skelterjohn/gcloud_apis/clients/bigtableclusteradmin/v1"
...
func Bigtableclusteradmin_v1_ProjectsAggregatedClustersList(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{+name}/aggregated/clusters", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
...
func Bigtableclusteradmin_v1_ProjectsZonesClustersUndelete(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{+name}:undelete", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
...
func Bigtableclusteradmin_v1_ProjectsZonesList(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("{+name}/zones", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
