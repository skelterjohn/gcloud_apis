...
import (
	...
	api_client "github.com/skelterjohn/gcloud_apis/clients/sourcerepo/v1"
...
func Sourcerepo_v1_ProjectsReposDelete(context Context, args ...string) error {
...
func Sourcerepo_v1_ProjectsReposGet(context Context, args ...string) error {
...
func Sourcerepo_v1_ProjectsReposList(context Context, args ...string) error {
	...
	usageFunc := func() {
		...
		if len(pathParams) != 0 {
			if strings.Contains("v1/{+name}/repos", "+") {
				usageBits += " @" + strings.Join(pathParams, "@")
			} else {
				usageBits += " " + strings.Join(pathParams, "/")
			}
